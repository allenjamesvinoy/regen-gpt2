...
def train_loop(model, optimizer, scheduler, scaler, device, train_loader, val_loader,
               train_config, model_config, lam=0.5, start_step=0, save_path = f'/content/drive/MyDrive/checkpoint_step{num_steps_train}_{datetime.now():%Y%m%d_%H%M%S}.pt'):
    ...
    save_checkpoint(model, optimizer, num_steps_train, final_loss, save_path)
