...
class BERT_Lag(nn.Module):
  ...
  """
  Two Forward Passes are provided. The first splits the batch in two. 
  Each half only is calculated on one loss.
  The second calculates both losses on both sequences.
  """
  ...
  #   y_bwd = y[half:][:, mask] if y is not None else None
  ...
  #   # Separate passes with different attention masks
...
class AttentionMultiHeadFused(nn.Module):
  ...
  def forward(self, x, mask=None):
    ...
    #The NTP task needs a causal mask but the BERT task cannot have one
    if(mask):
